...
export interface User {
  readonly id: UserID;
  readonly name: ElementType<UserList>;
}
export type IFriendshipPairs = [idx: number, idx: number][];
...
export type FoaF = {
  friends: UserID[];
  getAll: UserID[];
  getUnique: Set<UserID>;
  byCount: { [UserID: UserID]: number };
};
 ...
 */
export const friendshipLenById: [[idx: UserID, NumOfFriends: number]?] = [];
 ...
 */
export type IntrestsTypes = ElementType<typeof allIntrests>;
const allIntrests = Intrests.map((pair) => pair[1]);

/**
 * A couple that indicates that two indexes are friends
 */
export const friendship_pairs = [
  [0, 1],
  [0, 2],
  [1, 2],
  [1, 3],
